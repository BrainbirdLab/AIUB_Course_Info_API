...
def home():
    #return a random emoji
    return flask.Response(randomEmoji(), status=200, mimetype='text/html')


@app.route('/', methods=['POST'])
async def handleScraping():
    try:
        print('Processing request...')
        username = flask.request.form['UserName']
        password = flask.request.form['Password']
        ...
        if 'https://portal.aiub.edu' != response.url:
            #return flask.Response('Invalid username or password', status='401', mimetype='text/html')
            print('Login failed')
            return flask.Response(json.dumps({'success': False, 'message': 'Invalid username or password'}), status=401, mimetype='application/json')
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get('https://portal.aiub.edu/Student')
